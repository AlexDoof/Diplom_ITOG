...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Для Каждого ТекСтрокаНачисления Из Начисления Цикл
		Если ТекСтрокаНачисления.ВидРасчета = ПланыВидовРасчета.ВКМ_ОсновныеНачисления.Отпуск Тогда
			...
			Движение.Показатель = ТекСтрокаНачисления.Показатель;
			Движение.ГрафикРаботы = ТекСтрокаНачисления.График;
			Движение.ДниОтпуска = (КонецДня(ТекСтрокаНачисления.ПериодДействияКонец) - НачалоДня(
				ТекСтрокаНачисления.ПериодДействияНачало)) / 86400;
		КонецЕсли;
	КонецЦикла;
...
Процедура РассчитатьОтпуска()
	...
	Запрос.Текст = "ВЫБРАТЬ
				   ...
				   |ИЗ
				   |	РегистрРасчета.ВКМ_ОсновныеНачисления.ДанныеГрафика(Регистратор = &Ссылка) КАК ВКМ_ОсновныеНачисленияДанныеГрафика
				   |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрРасчета.ВКМ_ОсновныеНачисления.БазаВКМ_ОсновныеНачисления(&МассивИзмерений,
				   |			&МассивИзмерений, &РегистраторРазрезы, Регистратор = &Ссылка) КАК ВКМ_ОсновныеНачисленияБазаВКМ_ОсновныеНачисления
				   |		ПО ВКМ_ОсновныеНачисленияДанныеГрафика.НомерСтроки = ВКМ_ОсновныеНачисленияБазаВКМ_ОсновныеНачисления.НомерСтроки
				   |
				   |УПОРЯДОЧИТЬ ПО
				   |	НомерСтроки";
...
Процедура РассчитатьУдержания()
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрРасчета.ВКМ_Удержания.БазаВКМ_ОсновныеНачисления(&МассивИзмерений, &МассивИзмерений,
	|			&РегистраторРазрез, Регистратор = &Регистратор) КАК ВКМ_УдержанияБазаВКМ_ОсновныеНачисления
	...
	//Определение структуры для поиска по номеру строки    ***// Примечание : Количество записей в Выборке и в НаборЗаписей одинаковое!
