...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Процедура ПроверитьКонстанты_Номенклатура() Экспорт 
		...
		Если НоменклатураАбонентскаяПлата.Пустая() ИЛИ НоменклатураРаботыСпециалиста.Пустая() Тогда 
			ВызватьИсключение НСтр("ru = 'Не заполнена константа /НоменклатураАбонентскаяПлата/ или /НоменклатураРаботыСпециалиста/'");
		КонецЕсли;
	...
	Процедура ВКМ_ЗаполнитьНаСервере(Объект) Экспорт
		Если Объект.Договор.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.АбонентскоеОбслуживание Тогда   
			ПроверитьКонстанты_Номенклатура();	
		КонецЕсли;                  
		
		//Если в договоре ненулевая сумма абонентской платы...
		Если ЗначениеЗаполнено(Объект.Договор.ВКМ_АбонентскаяПлата) И Объект.Договор.ВКМ_АбонентскаяПлата <> 0 Тогда
			НоменклатураАбонентскаяПлата = Константы.ВКМ_НоменклатураАбонентскаяПлата.Получить();
			НоваяСтрока = Объект.Услуги.Добавить();
			НоваяСтрока.Номенклатура = НоменклатураАбонентскаяПлата;
			НоваяСтрока.Сумма = Объект.Договор.ВКМ_АбонентскаяПлата;
			ОбщегоНазначения.СообщитьПользователю("Абонентская плата за " + Формат(Объект.Дата, "ДФ='MMMM yyyy ''г.'''") + 
			" составила " + Объект.Договор.ВКМ_АбонентскаяПлата + " см. таблицу //Услуги//");
		КонецЕсли;
