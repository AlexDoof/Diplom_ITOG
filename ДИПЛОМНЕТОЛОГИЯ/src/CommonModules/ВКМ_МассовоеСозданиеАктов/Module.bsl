...
Функция СоздатьАктыПоДоговорамАбонентскойПлаты(Период, ДатаАктов) Экспорт   
	...
	// Создаем массив ссылок на договоры по которым в указанный период уже есть реализации
	Запрос = Новый Запрос;
	...
	ДоговорыРеализованные = Новый Массив;
	Выборка = Запрос.Выполнить().Выбрать(); 
	
	Пока Выборка.Следующий() Цикл
		ДоговорыРеализованные.Добавить(Выборка.Договор); 
	КонецЦикла;
	
	// Ищем ДоговорыСсылки которые не попадают в массив, созданный выше. И по этим договорам создаем и заполняем АВРы 
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	И НЕ ДоговорыКонтрагентов.Ссылка В (&МассивДоговоровРеализованных)
	               |	И ДоговорыКонтрагентов.ВКМ_НачалоДействияДоговора < &ДатаАкта
	               |	И ДоговорыКонтрагентов.ВКМ_КонецДействияДоговора > &ДатаАкта";
	
	Запрос.УстановитьПараметр("МассивДоговоровРеализованных", ДоговорыРеализованные);
